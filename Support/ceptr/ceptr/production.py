...
def production_rate(
    ...
    if n_reactions > 0:
        ...
        for orig_idx, idx in sorted(reaction_info.idxmap.items()):
            ...
            elif not falloff and not plog and not is_electron_temperature:
                # Case 2 !PD, TB, !PLOG
...
def production_rate_ele(
    ...
    # nspecial = ispecial[1] - ispecial[0]
    nelectron = ielectron[1] - ielectron[0]
    assert nelectron > 0, "There should be at least one electron temperature reaction when using -nlte"
    ...
    if n_reactions > 0:
        if n_qss_species > 0:
            cw.writer(
                fstream,
                "AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE void"
                " comp_qfqr_ele(amrex::Real *  qf, amrex::Real * qr, const"
                " amrex::Real * sc, const amrex::Real * sc_qss, const"
                " amrex::Real * T, const amrex::Real * invT, const amrex::Real * logT)"
            )
        else:
        ...
        cw.writer(
            fstream,
            "AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE void comp_qfqr_ele(amrex::Real *"
            " /*qf*/, amrex::Real * /*qr*/, const amrex::Real * /*sc*/, const"
            " amrex::Real * /*sc_qss*/, const amrex::Real* /*T*/, const amrex::Real*"
            " /*invT*/, const amrex::Real* /*logT*/)"
        )
    ...
    if n_reactions > 0:
        cw.writer(
            fstream,
            "AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE void "
            " productionRateEle(amrex::Real * wdot, const amrex::Real * sc,"
            " const amrex::Real T[NUM_TEMP])",
        )
    else:
        cw.writer(
            fstream,
            "AMREX_GPU_HOST_DEVICE AMREX_FORCE_INLINE void "
            " productionRateEle(amrex::Real * wdot, const amrex::Real *"
            " /*sc*/, const amrex::Real* /*T*/)",
        )
    cw.writer(fstream, "{")
